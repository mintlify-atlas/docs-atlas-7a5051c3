...
<ParamField path="request" type="SubscribeReplayInfoRequest">
  Empty request message.
</ParamField>

### Response

<ResponseField name="first_available" type="uint64" optional>
...
async function getReplayInfo() {
  ...
  if (response.firstAvailable) {
    console.log("First available slot for replay:", response.firstAvailable);
  } else {
    console.log("Historical replay not available");
  }
}

getReplayInfo().catch(console.error);
```
...
request = geyser_pb2.SubscribeReplayInfoRequest()
response = stub.SubscribeReplayInfo(request)

if response.HasField('first_available'):
    print(f"First available slot for replay: {response.first_available}")
else:
    print("Historical replay not available")
```
...
func main() {
    ...
    client := pb.NewGeyserClient(conn)
    request := &pb.SubscribeReplayInfoRequest{}
    
    response, err := client.SubscribeReplayInfo(context.Background(), request)
    if err != nil {
        log.Fatal(err)
    }

    if response.FirstAvailable != nil {
        fmt.Printf("First available slot for replay: %d\n", *response.FirstAvailable)
    } else {
        fmt.Println("Historical replay not available")
    }
}
```
